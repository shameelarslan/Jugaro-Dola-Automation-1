    ...
    async def run_session_batch(self, session: Dict[str, Any], jobs: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            for idx, job in enumerate(jobs):
                ...
                if start_result is True:
                ...
                elif start_result == "QUOTA_LIMIT_EXCEEDED":
                    self.current_stage = "Session Daily Limit Exceeded"
                    logger.error(f"🚨 [SESSION_DAILY_LIMIT_EXCEEDED] Session daily limit exceeded", category="WORKER", worker_id=self.worker_id, session_name=session_name)
                    db.make_session_expired(session_id)
                    db.update_job_status(job_id, "Failed", error_message="Session Daily Limit Exceeded")
                    ...
                    break
                elif start_result == "SESSION_EXPIRED":
                    self.current_stage = "Session Expired / Login Required"
                    logger.error(f"❌ Session '{session_name}' is EXPIRED / Logged Out. Aborting all remaining prompts for this session.", category="WORKER", worker_id=self.worker_id)
                    if len(context.pages) > 1:
                        try:
                            await page.close()
                        except Exception:
