...
class SessionSelectionDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(["Select", "Session ID", "Session Name", "Status"])
...
class AutomationView(QWidget):
    ...
    def _init_ui(self):
        ...
        btn_sub_row.addWidget(self.btn_pause)
        btn_sub_row.addWidget(self.btn_stop)

        btn_sub_row2 = QHBoxLayout()
        ...
        btn_sub_row2.addWidget(self.btn_reset_failed)
        btn_sub_row2.addWidget(self.btn_clear_completed)

        ctrl_layout.addWidget(self.btn_start)
        ctrl_layout.addLayout(btn_sub_row)
        ctrl_layout.addLayout(btn_sub_row2)
    ...
    def _save_settings(self):
        """Persists automation settings directly into SQLite DB."""
        self.config.default_model = self.combo_model.currentText()
        self.config.default_ratio = self.combo_ratio.currentText()
        self.config.total_sessions_to_run = self.spin_total_sessions.value()
        self.config.videos_per_session = self.spin_vps.value()
        self.config.sessions_at_a_time = self.spin_sat.value()
        self.config.concurrency_limit = self.spin_sat.value()
        self.config.enable_watermark_remover = self.chk_watermark.isChecked()
        self.config.blur_x = self.spin_blur_x.value()
        self.config.blur_y = self.spin_blur_y.value()
        self.config.blur_w = self.spin_blur_w.value()
        self.config.blur_h = self.spin_blur_h.value()
        ...
        cap = tot * vps
        try:
            pending_cnt = len(db.get_pending_prompts())
        except Exception:
            pending_cnt = 0
    ...
    def _on_pause_resume(self):
        if self.queue_manager._is_running and not self.queue_manager._is_paused:
            self.queue_manager.pause_automation()
            self.btn_pause.setText("▶ Resume")
        elif self.queue_manager._is_running and self.queue_manager._is_paused:
            self.queue_manager.resume_automation()
    ...
    def _on_reset_failed(self):
        ...
        QMessageBox.information(self, "Reset Complete", "All failed jobs have been reset to Pending.")
