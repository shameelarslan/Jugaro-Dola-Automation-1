...
class QueueView(QWidget):
    ...
    def _on_clear_queue_click(self):
        reply = QMessageBox.question(
            self,
            "Clear Job Queue",
            "Kya aap tamam Job Queue data clear karna chahte hain?",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.refresh_queue()
            QMessageBox.information(self, "Queue Cleared", "Job Queue aur status monitor clear ho chuke hain.")
    ...
    def refresh_queue(self):
        ...
        for row, j in enumerate(jobs):
            ...
            self.table.setItem(row, 1, QTableWidgetItem(j["prompt_text"]))
            self.table.setItem(row, 2, QTableWidgetItem(j["session_name"]))
