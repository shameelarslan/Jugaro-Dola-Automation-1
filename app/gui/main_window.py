...
from app.gui.views.viral_prompts_view import ViralPromptsView
...
from app.gui.dialogs.tutorial_dialog import WelcomeTutorialPromptDialog, InteractiveTourDialog
...
class MainWindow(QMainWindow):
    def __init__(self, queue_manager, parent=None):
        ...
        try:
            ...
            self._app_ver = get_installed_version()
        except Exception:
            self._app_ver = "2.0.6"

        self.setWindowTitle(f"Waqas's Automation Pro — v{self._app_ver}")
        ...
        is_admin_user = (cloud_manager.current_user or {}).get("role") == "admin"
        admin_manager.set_admin_mode(is_admin_user)
        self._on_admin_mode_changed(is_admin_user)
        ...
        # Check first-time onboarding tutorial 1.5s after boot
        QTimer.singleShot(1500, self._check_first_time_tutorial)
    ...
    def _init_ui(self):
        ...
        lbl_ver_badge = QLabel(f"v{self._app_ver}")
        ...
        """)
        header_layout.addWidget(lbl_ver_badge)
        ...
        self.sidebar.setMinimumHeight(370)
        ...
        btn_tutorial = QPushButton("❓  Interactive Tour & Guide")
        ...
        """)
        btn_tutorial.clicked.connect(self._open_tutorial_tour)
        util_box.addWidget(btn_tutorial)
        ...
        if user_role == "admin":
            lic_text = "👑 Super Admin"
            lic_style = "background-color: rgba(124, 77, 255, 0.2); color: #a78bfa; font-size: 10px; font-weight: 800; padding: 2px 6px; border-radius: 6px; border: 1px solid rgba(124, 77, 255, 0.4);"
        elif user_role == "paid":
            lic_text = "⭐ Pro Paid License"
            lic_style = "background-color: rgba(56, 189, 248, 0.2); color: #38bdf8; font-size: 10px; font-weight: 800; padding: 2px 6px; border-radius: 6px; border: 1px solid rgba(56, 189, 248, 0.4);"
        else:
            lic_text = "🟢 Free License"
            lic_style = "background-color: rgba(16, 185, 129, 0.15); color: #10b981; font-size: 10px; font-weight: 700; padding: 2px 6px; border-radius: 6px; border: 1px solid rgba(16, 185, 129, 0.35);"

        self.lbl_card_license = QLabel(lic_text)
        self.lbl_card_license.setStyleSheet(lic_style)
        ...
        self.view_viral_prompts = ViralPromptsView()
        ...
        self.stack.addWidget(self.view_viral_prompts)
    ...
    def _check_first_time_tutorial(self):
        """Shows welcome onboarding tutorial modal if user is running for the first time."""
        try:
            from app.core.config import cfg
            if not getattr(cfg, "tutorial_completed", False):
                dlg = WelcomeTutorialPromptDialog(self)
                result = dlg.exec()
                if result == 1:
                    self._open_tutorial_tour()
        except Exception as e:
            pass

    def _open_tutorial_tour(self):
        """Launches the step-by-step interactive feature tour."""
        try:
            tour = InteractiveTourDialog(self)
            tour.exec()
        except Exception as e:
            QMessageBox.critical(self, "Tour Error", f"Could not open interactive tour: {e}")
    ...
    def _on_tab_switched(self, index: int):
        ...
        if index == 0:
            ...
            # Refresh viral prompts list if new files were added
            self.view_viral_prompts._load_prompts_data()
        elif index == 6:
    ...
    def _on_admin_mode_changed(self, is_admin: bool):
        ...
        if hasattr(self, "sidebar"):
            ...
            self.sidebar.setRowHidden(7, not is_admin)
            if not is_admin and self.sidebar.currentRow() in [6, 7]:
                self.sidebar.setCurrentRow(0)
    ...
    def _check_for_updates(self):
        ...
        try:
            ...
            is_avail, release = updater.check_for_updates(user_email=user_email)
