...
class DashboardView(QWidget):
    ...
    def _init_ui(self):
        ...
        self.badge_ver = QFrame()
        self.badge_ver.setStyleSheet("""
            QFrame {
        ...
        """)
        ver_layout = QHBoxLayout(self.badge_ver)
        ver_layout.setContentsMargins(8, 3, 8, 3)
        ver_layout.setSpacing(6)
        
        lbl_ver_icon = QLabel("⚡")
        lbl_ver_icon.setStyleSheet("font-size: 11px; border: none; background: transparent;")
        self.lbl_ver_text = QLabel(f"v{curr_ver}")
        self.lbl_ver_text.setStyleSheet("color: #c7d2fe; font-weight: 800; font-size: 12px; border: none; background: transparent;")
        
        ver_layout.addWidget(lbl_ver_icon)
        ver_layout.addWidget(self.lbl_ver_text)
        ...
        status_card_layout.addSpacing(8)
        status_card_layout.addWidget(self.badge_ver)
        ...
        self.table_sessions.setHorizontalHeaderLabels(["Worker", "Session", "Stage", "Active Tabs", "Elapsed", "Actions"])
        self.table_sessions.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        ...
        self.table_sessions.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)
        self.table_sessions.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)
        self.table_sessions.horizontalHeader().setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)
    ...
    def _refresh_dashboard(self):
        ...
        # Active Session Instances Table (Zero-Lag Persistent Updates)
        worker_states = self.queue_manager.get_live_worker_states()
        cur_sig = tuple((w["worker_id"], w["session_name"], w["stage"], w.get("active_tabs", 0), w["elapsed_seconds"]) for w in worker_states)
        if hasattr(self, "_last_worker_sig") and cur_sig == self._last_worker_sig:
            return
        self._last_worker_sig = cur_sig

        self.table_sessions.setUpdatesEnabled(False)
        try:
            if self.table_sessions.rowCount() != len(worker_states):
            ...
            for r, w in enumerate(worker_states):
                ...
                # Column 0: Worker
                item_w = self.table_sessions.item(r, 0)
                if not item_w:
                    item_w = QTableWidgetItem(f"Worker {wid}")
                    self.table_sessions.setItem(r, 0, item_w)
                else:
                    item_w.setText(f"Worker {wid}")

                # Column 1: Session
                item_s = self.table_sessions.item(r, 1)
                if not item_s:
                    item_s = QTableWidgetItem(sname)
                    self.table_sessions.setItem(r, 1, item_s)
                else:
                    item_s.setText(sname)

                # Column 2: Stage
                item_st = self.table_sessions.item(r, 2)
                if not item_st:
                    item_st = QTableWidgetItem(w["stage"])
                    self.table_sessions.setItem(r, 2, item_st)
                else:
                    item_st.setText(w["stage"])
                ...
                if "Expired" in w["stage"] or "Login" in w["stage"]:
                    ...
                    item_st.setForeground(QColor("#3b82f6"))
                else:
                    item_st.setForeground(QColor("#e2e8f0"))

                # Column 3: Active Tabs
                item_tab = self.table_sessions.item(r, 3)
                if not item_tab:
                    item_tab = QTableWidgetItem(f"{w.get('active_tabs', 0)} Tabs")
                    self.table_sessions.setItem(r, 3, item_tab)
                else:
                    item_tab.setText(f"{w.get('active_tabs', 0)} Tabs")

                # Column 4: Elapsed
                item_el = self.table_sessions.item(r, 4)
                if not item_el:
                    item_el = QTableWidgetItem(f"{w['elapsed_seconds']}s")
                    self.table_sessions.setItem(r, 4, item_el)
                else:
                    item_el.setText(f"{w['elapsed_seconds']}s")

                # Column 5: Action Buttons (Persistent widget - no churn)
                has_active_session = sname and sname != "N/A" and w["stage"] != "Idle"
                can_terminate = has_active_session and w["stage"] not in ("Completed", "Terminated")

                act_widget = self.table_sessions.cellWidget(r, 5)
                if not act_widget:
                    act_widget = QWidget()
                    ...
                    btn_term = QPushButton("⏹ Close")
                    btn_term.setFixedHeight(24)
                    ...
                    act_layout.addWidget(btn_term)
                    act_widget.btn_open = btn_open
                    act_widget.btn_term = btn_term
                    self.table_sessions.setCellWidget(r, 5, act_widget)

                act_widget.btn_open.setEnabled(has_active_session)
                act_widget.btn_term.setEnabled(can_terminate)

                try:
                    act_widget.btn_open.clicked.disconnect()
                except Exception:
                    pass
                act_widget.btn_open.clicked.connect(lambda checked, _wid=wid, _sname=sname: self._on_open_worker_session(_wid, _sname))

                try:
                    act_widget.btn_term.clicked.disconnect()
                except Exception:
                    pass
                act_widget.btn_term.clicked.connect(lambda checked, _wid=wid, _sname=sname: self._on_terminate_worker_session(_wid, _sname))
        finally:
            self.table_sessions.setUpdatesEnabled(True)

    def _on_open_worker_session(self, worker_id: int, session_name: str):
        """Focuses the browser window for the running session and shows clear notification."""
        self.queue_manager.open_worker_session(worker_id)
        QMessageBox.information(
            self,
            "Active Session Inspector",
            f"🔍 Viewing Active Session:\n\n"
            f"📌 Session: {session_name}\n"
            f"🔧 Worker Slot: Worker {worker_id}\n\n"
            f"The browser window for this session has been brought to the foreground."
        )

    def _on_terminate_worker_session(self, worker_id: int, session_name: str):
        """Prompts confirmation and terminates only that worker's session."""
        reply = QMessageBox.question(
            self,
            "Terminate Running Session",
            f"Are you sure you want to terminate session '{session_name}' on Worker {worker_id}?\n\n"
            f"This will stop this session cleanly and free the slot for the rolling pool to pick up the next session.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.Yes:
            self.queue_manager.terminate_worker_session(worker_id)
            QMessageBox.information(
                self,
                "Session Terminated",
                f"✅ Session '{session_name}' on Worker {worker_id} has been terminated cleanly."
            )
