...
class UpdateDownloadWorker(QThread):
    ...
    def run(self):
        ...
        ok = updater.download_and_install_update(self.download_url, version_str=self.version_str, progress_callback=_cb)
...
class UpdateDialog(QDialog):
    ...
    def _start_download(self):
        ...
        self.worker_thread = UpdateDownloadWorker(download_url, self.release_info.get("version", ""))
    ...
    def _on_download_finished(self, success: bool):
        if success:
            ...
            # If detached batch script was already initiated, it will relaunch the updated executable
            if getattr(updater, 'is_detached_updater_active', False):
                from PyQt6.QtWidgets import QApplication
                QApplication.processEvents()
                # Brief sleep to allow UI update to render before closing
                import time
                time.sleep(0.5)
                sys.exit(0)
            
            # Dev mode fallback restart — must relaunch the ROOT main.py (project entry
            # point that sets up sys.path), not app/main.py, or `app.*` imports fail.
            try:
                if getattr(sys, 'frozen', False):
                    ...
                    subprocess.Popen([sys.executable, str(main_script)], cwd=str(project_root))
            except Exception as e:
                print(f"[WARNING] Auto-restart warning: {e}")
