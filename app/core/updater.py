...
try:
    ...
    version = None  # comparators fall back to integer tuple comparison
...
:: (exclude "data" so the zip's stale app_version.txt can never overwrite the live one)
if exist "{install_dir}\\_internal" (
    ...
    if exist "{source_content_dir}\\_internal" robocopy "{source_content_dir}\\_internal" "{install_dir}\\_internal" /E /R:5 /W:1 /XD "data" "logs" /XF "*.db" "*.db-wal" "*.db-shm" "user_session.json" /NJH /NJS /NDL /NC /NS
)

:: Write the new version LAST, to every location the app may read it from.
:: The frozen app resolves its version file inside _internal\\data, so both must be stamped.
if not exist "{install_dir}\\data" mkdir "{install_dir}\\data"
echo {new_ver}> "{install_dir}\\data\\app_version.txt"
if exist "{install_dir}\\_internal" (
    if not exist "{install_dir}\\_internal\\data" mkdir "{install_dir}\\_internal\\data"
    echo {new_ver}> "{install_dir}\\_internal\\data\\app_version.txt"
)
...
"""
                ...
                for root, dirs, files in os.walk(source_content_dir):
                    # _internal/ only applies to frozen installs; keep it out of the source tree
                    ...
                    for f in files:
                        if f.endswith((".db", ".db-wal", ".db-shm")) or f == "user_session.json":
                            continue
                        # The zip may carry the build machine's stale app_version.txt —
                        # the real version is stamped by set_installed_version() below.
                        if f == "app_version.txt":
                            continue
