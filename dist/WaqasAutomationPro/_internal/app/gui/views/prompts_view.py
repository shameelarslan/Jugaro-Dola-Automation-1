...
class PromptsView(QWidget):
    def __init__(self, parent=None):
        ...
        self._search_timer.timeout.connect(lambda: self.load_prompts(force=True))
    ...
    def _init_ui(self):
        ...
        self.lbl_count = QLabel("0 Prompts in Database")
        ...
        btn_refresh.clicked.connect(lambda: self.load_prompts(force=True))
        ...
        grp_paste = QGroupBox("Quick Paste Prompts (One per line — Supports 500+ Prompts Bulk Add)")
        ...
        self.txt_bulk.setPlaceholderText("Paste multiple prompts here (one prompt per line)...")
        ...
        self.txt_search.setPlaceholderText("🔍 Filter prompts by text or status (Instant search)...")
        self.txt_search.textChanged.connect(lambda: self._search_timer.start(250))
    ...
    def load_prompts(self, force: bool = False):
        """Fetches prompts using high-speed indexed query and batch-populates table with zero lag."""
        if not force and not self.isVisible():
            return

        filter_text = self.txt_search.text().strip().lower()

        # Ultra-fast indexed fetch
        rows = db.get_prompts_for_library()

        # Build light signature for fast change detection
        sig = (len(rows), rows[0]["id"] if rows else None, rows[0]["j_status"] if rows else None, filter_text)
        if not force and sig == self._last_data_sig:
            return
        ...
        self.lbl_count.setText(f"{len(rows)} Total Prompts in Database" + (f" (Showing {len(filtered_rows)})" if filter_text else ""))

        # Freeze table redraws during population for maximum performance
    ...
    def _on_add_pasted(self):
        ...
        if not lines:
            QMessageBox.warning(self, "Input Empty", "No valid prompt lines detected.")
            return

        # High-speed bulk atomic insert (executemany)
        added_count = db.add_prompts_bulk(lines)

        self.txt_bulk.clear()
        self.load_prompts(force=True)
        QMessageBox.information(self, "Prompts Added", f"⚡ Added {added_count} prompt(s) to Library in bulk instantly!")

    def _on_import_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Import Prompts File", "", "Supported Files (*.txt *.csv *.xlsx *.xls);;Text Files (*.txt);;CSV Files (*.csv);;All Files (*.*)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            prompts = PromptManager.import_from_file(file_path)
            if not prompts:
                QMessageBox.warning(self, "Empty File", "No prompts found in the selected file.")
                return

            added_count = db.add_prompts_bulk(prompts)
            self.load_prompts(force=True)
            QMessageBox.information(self, "Import Complete", f"⚡ Successfully imported {added_count} prompt(s) from {Path(file_path).name}!")
    ...
    def _on_reset_failed(self):
        ...
        if count > 0:
            self.load_prompts(force=True)
            QMessageBox.information(self, "Reset Complete", f"🔄 Successfully reset {count} failed prompt(s) to Pending status.")
        else:
            QMessageBox.information(self, "Notice", "There are currently no Failed prompts in the library.")

    def _on_clear_completed(self):
        count = db.clear_completed_prompts_and_jobs()
        if count > 0:
            self.load_prompts(force=True)
            QMessageBox.information(self, "Cleaned", f"🧹 Successfully deleted {count} completed prompt(s) from library.")
        else:
            QMessageBox.information(self, "Notice", "There are currently no Completed prompts in the library.")
